...
In case the title page template of these instructions can't be used, you can produce the title page yourself following the model available from the degree programme web pages. Please ensure that there are no typos, and that your degree programme is stated correctly. Your name should be written into the middle. The title should be written below your name with capital letters, centered, and divided into several lines as necessary to produce a balanced appearance. University logo and the faculty name are placed at the top and the type of the thesis, degree programme name, and date are placed at the bottom.
...
The bibliographic information of the thesis should be printed at the top of the abstract page. The keywords of your thesis should be printed below the abstract. The recommended number of keywords is 2--6 keywords or word sets. It is recommended that the keywords are not any words included in the title of the thesis. Keywords serve an important purpose for anyone performing literature searches in a library and other information catalogues. For that reason, prefer established subject headings to phrases of your own: the IEEE Thesaurus~\cite{ieeethesaurus} for English and the General Finnish Ontology YSO~\cite{yso} for Finnish. List the keywords in alphabetical order, as the IEEE style~\cite{ieeetransactions} does for index terms. An example abstract is presented in Appendix 1.
...
`b' is bits) and 1024 bytes is one kibibyte, 1\,KiB. Similarly, 1\,048\,576 bytes mebi (Mi); the larger units are gibi (Gi), tebi (Ti), pebi
(Pi), and exbi (Ei).

In the list, the abbreviations are defined first, then mathematical and other symbols, and finally letter symbols, so that Latin, Greek, and other letters are each presented in their designated groups. 
...
thesis work relies. However, unnecessary wordiness must be avoided and as a result the theory presented must be directly linked to the realized work. It is noteworthy, that many theses do not include a theoretical section of any type. \textbf{Hence, you should not insist on adding theory if it does not fit in naturally.}
...
The decimal separator differs between the two thesis languages: a thesis written in Finnish takes a comma (3,14), one written in English a point (3.14). The template does not switch this for you, so pick the marker of your thesis language and use it consistently. Tables are typeset as text rather than pasted in as images, so their numbers are yours to format; only a figure reproduced from another source keeps the marker it came with. Group the digits of long numbers with a thin space rather than a comma (1\,048\,576, typed \verb|1\,048\,576|), because the comma is the decimal marker in Finnish. Numbers of four digits are normally left ungrouped.

In steady movement, speed $v$ is
% Punctuation after a display equation goes inside \text{} so that it is
% set upright and spaced as text rather than as a math symbol.
\begin{equation}
    v = \frac{s}{t}\text{,}
    \label{eq:speed}
\end{equation}
