...
unnecessary excessive use of words}---a thesis usually has \DIFdelbegin \DIFdel{50-80 }\DIFdelend \DIFaddbegin \DIFadd{50--80
}\DIFaddend pages. The language (English/Finnish/ Swedish) should be fluent and
...
%DIFDELCMD < \setbox1%%%
\DIFdel{=}%DIFDELCMD < \hbox{$\bullet$}\setbox2%%%
\DIFdel{=}%DIFDELCMD < \hbox{\tiny$\bullet$}
%DIFDELCMD < \setlength{\mylen}{\dimexpr0.7\ht1-0.5\ht2}
%DIFDELCMD < \renewcommand\labelitemi{\raisebox{\mylen}{\tiny$\bullet$}}
%DIFDELCMD < \renewcommand\labelitemii{\raisebox{\mylen}{\tiny$\bullet$}}
%DIFDELCMD < %%%
\DIFdelend 
...
\begin{itemize}
  ...
  \item Font: \DIFdelbegin \DIFdel{Latin Modern
    }\DIFdelend \DIFaddbegin \DIFadd{Times New Roman
  }\DIFaddend \item Page margin settings \DIFdelbegin \DIFdel{:
    }\DIFdelend \DIFaddbegin \DIFadd{(exception: physical, bound copies use different settings; see Section~\ref{boundversion}):
    }\DIFaddend \begin{itemize}[wide=0pt]
        \setlength\itemsep{0pt}
        \setlength\parskip{0pt}
      \item Left margin: \DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{2}\DIFaddend ,5 cm
      \item Right margin: 2,\DIFdelbegin \DIFdel{0 }\DIFdelend \DIFaddbegin \DIFadd{5 }\DIFaddend cm
...
%DIFDELCMD <     %%%
\DIFdelend \item Line spacing: the default value for each font size, which
        is usually the font size \DIFdelbegin \DIFdel{+
    }\DIFdelend 2 pts.
      \item \DIFdelbegin \DIFdel{A single row should be left between chapters. The 1\textsuperscript{st} paragraph after a
        headingshould not be indented. Subsequent paragraphs should be }\DIFdelend \DIFaddbegin \DIFadd{The space between two paragraphs is a normal line spacing
      }\item \DIFadd{There is no indent in the first paragraph following a
        heading, but the following
        paragraphs are }\DIFaddend indented by 0.5 cm
      \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \item \DIFadd{A single empty line is added between table and figure
...
\begin{itemize}
    \setlength\itemsep{0pt}
    \setlength\parskip{0pt}
  \item \DIFdelbegin \DIFdel{A single empty row should be left between text and the figure or table caption. 
    }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdelend Table structure and the different fonts used in different
  ...
  \item In tables, the heading has to be placed above the table. The
    ...
    publish a table copied \DIFdelbegin \DIFdel{form }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend another source has to be mentioned as
    ...
    granted. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{The figure caption text is positioned underneath the table and the caption text ends
    in a full stop. There should be no references in a captiontext}\DIFdelend \DIFaddbegin \DIFadd{The table must be in text format – not an image.
  }\item \DIFadd{If necessary, the caption should be separated from the figure
    ...
    added to the end of the caption}\DIFaddend . The permission to publish a
    ...
    this permission when such a permission has been granted.  \DIFaddbegin \DIFadd{The
    copyright issues and citation practices of images are further
    discussed in~}\secref{copyright}\DIFadd{.
  }\DIFaddend \item Do not start a chapter with a figure, but embed it in the
...
\DIFdelbegin %DIFDELCMD < \captionsetup[table]{justification=raggedright,singlelinecheck=off}
%DIFDELCMD < %%%
...
plenty of figures. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \figref{fig:work_packages} serves as an example of a figure and a
...
\DIFdelbegin \DIFdel{According to the copyright enactments, you must always have the permission from the copyright holder to use the
figure in your work. }\textbf{\DIFdel{Copyright infringement may lead to legal actions and penalties, and in those cases the author is responsible.}} %DIFAUXCMD
\DIFdel{The writer should grow towards to mainly using figures of his own in the thesis. }\textbf{%DIFDELCMD < \figref{fig:work_packages} %%%
\DIFdel{serves as an example of authors' own production, thus requiring no reference or footnotes.}} %DIFAUXCMD
\DIFdel{In other words: if no outside source is
referred to , it is assumed that the author of the thesis is also the author of the figure.
}\DIFdelend 

\DIFdelbegin \DIFdel{It is also good practice to
explicitly state a license for your
images, such as CC BY. Please refer to the university instructions on
further instructions on copyright issues. }\DIFdelend %DIF >  Pictures in .pdf, .png or .jpg. No need to specify the extension.
%DIF >  Transparent images are not allowed with PDF/A.
%DIF >  With ImageMagick transparency can be removed with
%DIF >  convert image.png -background white -alpha remove -alpha off image_nobg.png
%DIF >  This restriction also affects PDF images, as a first step try to make the PDF
%DIF >  image PDF/A:
%DIF >  gs -dPDFA -dBATCH -dNOPAUSE
%DIF >  -sColorConversionStrategy=UseDeviceIndependentColor
%DIF >  -sDEVICE=pdfwrite -dPDFACompatibilityPolicy=2
%DIF >  -sOutputFile=output.pdf input.pdf
\DIFaddbegin \begin{figure}[ht]
\begin{center}
  \includegraphics*[width=0.7\textwidth]{WP}
\end{center}
\caption{\DIFaddFL{Connections between work packages (WP).
  }\copyrightstring\DIFaddFL{\ }\href{https://creativecommons.org/licenses/by/4.0/}{\DIFaddFL{CC
BY 4.0}}\DIFaddFL{.}}
...
\DIFaddbegin \DIFadd{If no external source is mentioned in the caption, it is assumed that
...
further discussed in }\secref{copyright}\DIFadd{. For figures that constitute
...
\DIFadd{Figures and tables should be given an alternative text that summarises
the information they contain for the reader. This is discussed further
in~}\secref{accessibility}\DIFadd{. Unfortunately support for this in
}\LaTeX\DIFadd{\ is still work in progress, and the alternative text will
currently not end up in the final PDF, so including them is not a requirement.
A suitable alternative text for~}\figref{fig:work_packages} \DIFadd{could be:
}

\begin{quote}
\DIFadd{Diagram showing interactions between work packages WP1 to WP5, all
...
\DIFaddend The final thesis will be published as a PDF/A file, and this LaTeX
template produces files in that format directly\DIFdelbegin \DIFdel{(There is no need to
use Muuntaja)}\DIFdelend . Be sure to remove all
transparency in your source images to ensure the end result is also
compliant. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Pictures in .pdf, .png or .jpg. No need to specify the extension.
%DIF <  Transparent images are not allowed with PDF/A.
%DIF <  With ImageMagick transparency can be removed with
%DIF <  convert image.png -background white -alpha remove -alpha off image_nobg.png
%DIF <  This restriction also affects PDF images
%DIFDELCMD < \begin{figure}[ht]
%DIFDELCMD <   \begin{center}
%DIFDELCMD <     \includegraphics*[width=0.7\textwidth]{WP}
%DIFDELCMD <   \end{center}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Connections between work packages (WP). \copyright\ Unto Uneksija 2025 CC BY}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:work_packages}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As mentioned, in case the figure has been taken from elsewhere, the author needs to have a permission from the copyright holder. Usually this is based on a licence that gives permission to use the work with some conditions. The most common example would be a CC-BY licence, such as the one used in }%DIFDELCMD < \figref{fig:ccbypic}%%%
\DIFdel{.  Note how the footnote is used instead of a citation to point to the original source}\DIFdelend \DIFaddbegin \DIFadd{This affects figures in PDF and PNG formats. Instructions
on doing this are included with the thesis template.  If you have done
this for all of your images, there is no need to use Muuntaja}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \begin{center}
%DIFDELCMD <     \includegraphics[width=12cm]{Figures/ccby.jpg}
%DIFDELCMD <   \end{center}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption[]{%
{%DIFAUXCMD
\DIFdelFL{"Stephen Crane, Author, Red Badge of Courage" by Tony Fischer Photography}%DIFDELCMD < \footnotemark%%%
\DIFdelFL{.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:ccbypic}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\footnotetext{\DIFdel{"Stephen Crane, Author, Red Badge of Courage", available at https://openverse.org/image/110032f8-1a7a-421f-86b1-88fdfde2e44f by Tony Fischer Photography is licensed under CC BY 2.0}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Algorithms}
...
%DIF < For more documentation and examples of creating algorithms, see: http://ftp.acc.umu.se/mirror/CTAN/macros/latex/contrib/algorithm2e/doc/algorithm2e.pdf
%DIF > For more documentation and examples of creating algorithms, see:
%DIF >  http://ftp.acc.umu.se/mirror/CTAN/macros/latex/contrib/algorithm2e/doc/algorithm2e.pdf
\vspace{7mm}
\begin{algorithm}[H]
\DIFdelbegin %DIFDELCMD < \ifthenelse{\equal{\getlanguage}{finnish}}
%DIFDELCMD <     {\renewcommand{\algorithmcfname}{Algoritmi}}
%DIFDELCMD <     {}
%DIFDELCMD < %%%
...
in a subsection titled \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend \textit{Author's Contributions and the Role
of Artificial Intelligence}.\DIFdelbegin \DIFdel{" }\DIFdelend \DIFaddbegin \DIFadd{'' }\DIFaddend This subsection serves two purposes.
...
%DIF < Pictures in .eps if you use latex, .pdf or .png if you use pdflatex. Don't specify the extension so you can use both!
%DIF > Pictures in .eps if you use latex, .pdf or .png if you use pdflatex.
%DIF >  Don't specify the extension so you can use both!
\begin{figure}[ht]
\begin{center}
...
Most word processors, \LaTeX\DIFaddbegin \DIFadd{\ }\DIFaddend included, have a spelling checker
...
\DIFdelbegin \DIFdel{Moreover, each student can her/himself decide whether the thesis will be }\DIFdelend \DIFaddbegin \DIFadd{All theses in the University of Oulu that are to be approved are
}\DIFaddend analyzed with a program called \DIFdelbegin \DIFdel{Urkund}\DIFdelend \DIFaddbegin \DIFadd{Turnitin}\DIFaddend . This program checks the
...
in the Internet). The role of \DIFdelbegin \DIFdel{Urkund }\DIFdelend \DIFaddbegin \DIFadd{Turnitin }\DIFaddend is twofold: First, it checks
...
the thesis is in \DIFdelbegin \DIFdel{Urkund}\DIFdelend \DIFaddbegin \DIFadd{Turnitin}\DIFaddend ’s databases, it checks whether others have
copied text from this thesis.

\DIFdelbegin \DIFdel{We recommend allowing the thesis it be delivered to Urkund }\DIFdelend \DIFaddbegin \DIFadd{The student can choose whether their thesis is stored as comparison material
in Turnitin. We recommend doing this, }\DIFaddend as then
the thesis is protected against incorrect copying by others. However,
\DIFdelbegin \DIFdel{as thesis }\DIFdelend once delivered to \DIFdelbegin \DIFdel{Urkund }\DIFdelend \DIFaddbegin \DIFadd{Turnitin the thesis }\DIFaddend cannot be deleted from its
databases\DIFdelbegin \DIFdel{, the text }\DIFdelend \DIFaddbegin \DIFadd{. Text }\DIFaddend copied from others’ documents must be referenced
and quoted properly in the first version delivered to \DIFdelbegin \DIFdel{Urkund}\DIFdelend \DIFaddbegin \DIFadd{Turnitin}\DIFaddend .
Hence, a student must \textbf{always} check correct referencing and
quoting with the supervisor before delivering the work to \DIFdelbegin \DIFdel{Urkund}\DIFdelend \DIFaddbegin \DIFadd{Turnitin}\DIFaddend !
...
\DIFdelbegin \DIFdel{More information can be found }\DIFdelend \DIFaddbegin \DIFadd{The University of Oulu offers a free tool, Turnitin Draft Coach, to
...
}\url{https://www.oulu.fi/en/news/want-improve-your-academic-writing-and-achieve-better-results}\DIFadd{.
}

\DIFadd{Students can utilize the Turnitin similarity checking independently.
The tool can be found in Moodle
}\url{https://moodle.oulu.fi/course/view.php?id=26689} \DIFadd{(Course key:
Turnitin2025). The thesis is checked individually, so the originality
of the text can be confirmed.
}

\DIFadd{More information can be found }\DIFaddend from the degree programme web pages
\DIFaddbegin \DIFadd{~}\DIFaddend \cite{mscstudies} and from Laturi’s web pages: \DIFdelbegin \DIFdel{http://laturi.
oulu.fi.
}\DIFdelend \DIFaddbegin \url{http://laturi.oulu.fi}\DIFadd{.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{boundversion}
\DIFaddend It is not mandatory to bind the thesis at a printing house. However, a
student is free to bind the thesis when she/he wants to have a
high-quality copy for relatives, for example.
\DIFdelbegin \DIFdel{For the bound thesis}\DIFdelend \DIFaddbegin 

\DIFadd{For bound theses, select the }\textit{\DIFadd{print}} \DIFadd{option in the thesis
template, which adjusts the page margins to the following: left 4}\DIFaddend ,\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{5
...
\DIFadd{To ensure a high-quality thesis that is consistent with previous
theses, the following rules apply: The }\DIFaddend words MASTER’S THESIS should be
printed in the \DIFdelbegin \DIFdel{middle }\DIFdelend \DIFaddbegin \DIFadd{center }\DIFaddend of the front cover. The NAME OF THE AUTHOR
\DIFdelbegin \DIFdel{of the thesis }\DIFdelend should be printed in the lower \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend corner of the front
cover. The NAME OF THE AUTHOR and \DIFaddbegin \DIFadd{the }\DIFaddend year of publication should be
printed \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the spine of the thesis. \DIFdelbegin \DIFdel{You must have your thesis }\DIFdelend \DIFaddbegin \DIFadd{Your thesis must be }\DIFaddend bound at a
...
\DIFdelbegin \DIFdel{have brought with you}\DIFdelend \DIFaddbegin \DIFadd{provide. The cover must be black, and the text must be printed
directly on it; stickers are not allowed.
}

\section{\DIFadd{Copyright, Licenses and the Use of images}}
\label{copyright}
\DIFadd{Intellectual property rights (IPR) protect works, i.e., the authors
...
\DIFadd{Your thesis is also a copyrighted work. If no license is explicitly
stated, the license is }\enquote{All rights reserved,} \DIFadd{meaning it can only be
...
or if warranted, another CC license~\mbox{%DIFAUXCMD
\cite{coordination_open_2024}}\hskip0pt%DIFAUXCMD
.
CC-BY 4.0 allows reuse, including commercial use, of your work as long as
the author is credited.
}

\DIFadd{Among the various types of works protected by copyright, images
...
\DIFadd{According to the copyright enactments, you must always have
permission from the copyright holder, which is not necessarily the
author, to use their figure in your work. To summarize:
}\textbf{\DIFadd{Copyright infringement may lead to legal actions and
penalties, and in those cases the author is responsible.}} \DIFadd{The writer
...
\DIFadd{It is good practice to explicitly state the copyright owners and
...
to the main license of your thesis (e.g., }\enquote{All rights reserved} \DIFadd{or
}\enquote{CC-BY 4.0}\DIFadd{) should }\textbf{\DIFadd{always}} \DIFadd{be included in the image
captions.
}

\DIFadd{If your use of external images is extensive and is a central part of
your thesis, it is good to also explicitly list them on the copyright
page or in an appendix, e.g., ``Figure 5 is reproduced from }[\DIFadd{x}] \DIFadd{under
...
\DIFadd{The starting point is that the figures must be related to the text,
...
\DIFadd{It is a good practice to mark the photographer in the photos
...
\DIFadd{As mentioned, in case the figure has been taken from elsewhere, the
...
such as the one used in }\figref{fig:ccbypic}\DIFadd{. If the figure is from a
publication you are citing, ``Under/Reprinted/Adapted CC BY 4.0 license
picture from }[\DIFadd{x}] \DIFadd{\textcopyright 20xx Author.'' would be
appropriate. In Finnish, use ``}[\DIFadd{Muokattu}] \DIFadd{CC BY 4.0 lisenssin alainen
kuva lähteestä }[\DIFadd{x}]\DIFaddend . \DIFdelbegin \DIFdel{The color of the cover has to be black. The text has to be printed on the cover, i.e., stickers should not be applied!
 }\DIFdelend \DIFaddbegin \DIFadd{\textcopyright Vuosi Tekijä.''
}
...
\end{center}
\caption{\DIFaddFL{``}\href{https://openverse.org/image/110032f8-1a7a-421f-86b1-88fdfde2e44f
  }{\DIFaddFL{Stephen Crane, Author, Red Badge of Courage}}\DIFaddFL{''
  by~}\href{https://www.flickr.com/photos/tonythemisfit/}{\DIFaddFL{Tony Fischer
  Photography}} \DIFaddFL{is licensed under
}\href{https://creativecommons.org/licenses/by/2.0/}{\DIFaddFL{CC BY 2.0}}\DIFaddFL{.}}
\label{fig:ccbypic}
\end{figure}

\DIFadd{The permission can also be from the publisher. For instance, IEEE and
...
permission, from }[\DIFadd{x}]\DIFadd{.''
}

\DIFadd{Not all figures exceed the threshold of originality. These include,
...
caption (}\enquote{Figure from [x].}\DIFadd{). However, it would be best to ask
for permission or at least redraw the image. Images with a CC license
are free to use, so long as the terms of use are
followed~\mbox{%DIFAUXCMD
\cite{about_cc_licenses}}\hskip0pt%DIFAUXCMD
. Even then, the caption must mention
the source (article, website, etc.) and that the image is CC-licensed.
}

\DIFadd{Based on the right to quote, it is possible to take a photocopy,
...
\DIFadd{Sometimes you may want to use someone else's original image idea as a
...
}[\DIFadd{x}]\DIFadd{'') and must be declared in the caption. Should you only make minor
edits to the original, separate permission is required. The University
of Oulu Acta Oulu publishing guide~\mbox{%DIFAUXCMD
\cite{ronkainen_copyright} }\hskip0pt%DIFAUXCMD
has more
details on this topic, and also the proper wordings in
Finnish~\mbox{%DIFAUXCMD
\cite{ronkainen_tekijanoikeus}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{When searching for images through an internet image search, it is not
...
\section{\DIFadd{Accessibility}}
\label{accessibility}
\DIFadd{Put effort into the accessibility of your thesis. The
...
Oulu Campus ICT~\mbox{%DIFAUXCMD
\cite{ictaccessibleword,ictwordpdfa}}\hskip0pt%DIFAUXCMD
. You can also
read about the accessibility of mathematical equations in the Campus
ICT text~\mbox{%DIFAUXCMD
\cite{ictaccessiblemath}}\hskip0pt%DIFAUXCMD
.
 }\DIFaddend