...
\begin{table}[!ht]
  ...
  \begin{center}
    ...
    % Tag the header cells so screen readers announce them as headers rather
    % than as ordinary data cells (see \secref{accessibility}). The heading of
    % this table spans the first three rows, and the leftmost column holds the
    % row headings. The setting is local to the table environment.
    \tagpdfsetup{table/header-rows={1,2,3}, table/header-columns={1}}
...
prior to writing the thesis, and familiarize yourself with the
...
\begin{center}
  \includegraphics*[width=0.9\textwidth,
    alt={Photograph of a student seen from behind, writing at a desktop
      computer in an office, with crumpled paper and empty coffee cups piled
      on the desk. Two comic-style speech bubbles carry the text quoted in
      the caption.}]{unto_uneksija}
\end{center}
...
Set the checker to one variety of English, British or American. Either
is acceptable, but a thesis should not mix the two, and a mixture is
easy to introduce in a text that is drafted over many months. The same
applies to the spelling of recurring technical terms: choose a form and
keep it.
...
should grow toward to mainly using figures of his own in the
...
\begin{center}
  \includegraphics[width=12cm,
    % Note: alt text is written verbatim into the PDF, so quotes and dashes
    % must be typed as the characters you want, not as LaTeX shorthands.
    alt={Photograph of a flat granite grave marker set in grass, inscribed
      “Stephen Crane, Author, 1871–1900”.}]{Figures/ccby.jpg}
\end{center}
...
\end{verbatim}
Alternative text is written into the PDF as it is typed, so type quotation
marks and dashes as the characters you want rather than as the \LaTeX{}
shorthands \verb|``|, \verb|''| and \verb|---|. Describe what the figure
shows, not what the caption already says, and keep the description of the
same thing consistent if the figure is reused.

A table needs its heading cells marked so that a screen reader can announce
which column and row a value belongs to instead of reading a flat sequence
of numbers. Give the numbers of the heading rows and columns before the
table, as in \tabref{tab:sample_table}:
\begin{verbatim}
\tagpdfsetup{table/header-rows={1}, table/header-columns={1}}
\end{verbatim}
The setting applies to the next table only if it is given inside the
\texttt{table} environment; a \texttt{longtable} is not a float, so reset it
afterwards with an empty value. In a \texttt{longtable} that repeats a
heading via \verb|\endhead|, the heading rows are detected automatically.
Keep tables simple---merged cells, cells left empty for layout, and a single
logical row split across several \verb|\\| lines all reach the screen reader
as the structure you typed rather than the one the sighted reader sees.
