...
\DocumentMetadata{
  lang        = en, % Main language of the thesis; must match \documentclass.
                    % \DocumentMetadata runs before \documentclass, so it
                    % cannot pick this up on its own and warns if it is unset.
  % Output PDF/A-4f and UA-2
  pdfversion  = 2.0,
...
% Keywords are also included in the PDF metadata (pdfkeywords), separated by
% commas. The \texorpdfstring{\footnote{...}}{} shows guidance on the abstract
% page only; the empty PDF-string branch keeps it out of the metadata. Remove
% the footnote in your own thesis.
%
% List the keywords in alphabetical order, as the IEEE Editorial Style Manual
% for Authors does for index terms. Prefer established subject headings to ad
% hoc phrases: the IEEE Thesaurus
% (https://www.ieee.org/publications/services/thesaurus.html) for English, the
% General Finnish Ontology YSO (https://finto.fi/yso/) for Finnish.
\keywords{M.Sc. degree, master’s thesis instructions, structure of a master’s
  thesis\texorpdfstring{\footnote{The keywords should differ from those used in
  the title. Remove this footnote from your thesis.}}{}}
\avainsanat{diplomi-insinöörin tutkinto, diplomityön ohjeet, diplomityön
  rakenne\texorpdfstring{\footnote{Käytä avainsanoina eri sanoja kuin lopputyön
  otsikossa. Poista tämä alaviite lopputyöstäsi.}}{}}
